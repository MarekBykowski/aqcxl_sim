...
sub vcs_execute {
    ...
    $pli .= " $AVERY_PLI/lib.linux/libtb_vcs64.so";
    $pli_sim .= " -svlib $AVERY_PLI/lib.linux/libtb_vcs64";
    if (!$is_release_version) {
    ...
    $simulate .= "; ./simv $vcs_sim_opts -sv_lib $pli_lib $pli_sim +ntb_random_seed=$opt_n -do $vcs_do $runtime";

    $cmd .= "vcs $vcs_opts $pli $pli_tb ${pli_lib}.so $vlog_stuff $compile -l vcs_compile.log";
    $cmd .= " $simulate ";
...
sub mti_execute {
    ...
    $pli .= " -sv_lib $qemu_pli";
    if (!$is_release_version) {
        $pli .= " -sv_lib $AVERY_QEMU/dpi/measure/libmeasure64";
    } else {
        $pli .= " -sv_lib $AVERY_QEMU/../aqemu/lib/libmeasure64";
    }
